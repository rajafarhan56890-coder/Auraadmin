...
/* =========================================================
   COMBO MEALS
   ---------------------------------------------------------
   Bundles existing products into a discounted combo. Stored in its own
   "combos" Firestore collection (nothing about products/categories/orders/
   users/etc. is touched). Each combo document keeps:
     name, description, originalPrice, comboPrice, imageUrl,
     productIds (the selected products' ids),
     items (a denormalized [{id,name}] snapshot of those products' names,
       taken at save time so the storefront can show "Includes: ..." without
       needing a live products join),
...
function renderCombosTable() {
  ...
  $('combos-table-body').innerHTML = rows.length === 0
    ...
    : rows.map((c) => {
      ...
      const savings = Math.max(original - comboPrice, 0);
      const itemCount = Array.isArray(c.productIds) ? c.productIds.length : 0;
      return `
      <tr>
        ...
        <td class="mono">${money(original)}</td>
        <td class="mono">${money(comboPrice)}</td>
        <td class="mono herb">${money(savings)}</td>
        <td>${c.featured ? '<span class="badge badge-active">Featured</span>' : '—'}</td>
        <td><span class="badge ${c.active !== false ? 'badge-active' : 'badge-inactive'}">${c.active !== false ? 'Active' : 'Hidden'}</span></td>
        <td style="white-space:nowrap;">
...
// Same checkbox-list pattern as sectionProductPickerHTML (Homepage Sections),
// but simpler — no drag-to-reorder needed for a combo's product list.
function comboProductPickerHTML(selectedIds) {
  ...
  return productsCache.map((p) => `
    <label class="sf-product-row" data-name="${esc((p.name || '').toLowerCase())}">
      <input type="checkbox" class="cf-product-check" value="${p.id}" ${selectedIds.includes(p.id) ? 'checked' : ''} />
      <span class="sf-product-name">${esc(p.name)}</span>
...
function comboFormHTML(c = {}) {
  ...
  return `
    <div class="field"><label>Combo Name</label><input id="cf-name" value="${esc(c.name || '')}" /></div>
    <div class="field"><label>Combo Description</label><textarea id="cf-description" rows="2">${esc(c.description || '')}</textarea></div>
    <div class="form-row">
      <div class="field"><label>Original Price (PKR)</label><input id="cf-original-price" type="number" step="0.01" value="${c.originalPrice ?? ''}" /></div>
      <div class="field"><label>Combo Price (PKR)</label><input id="cf-combo-price" type="number" step="0.01" value="${c.comboPrice ?? ''}" /></div>
    </div>
    <div class="field"><label>You Save</label><input id="cf-savings" type="text" value="${money(Math.max((Number(c.originalPrice) || 0) - (Number(c.comboPrice) || 0), 0))}" disabled /></div>
    <div class="field">
      <label>Combo Image</label>
      <img class="image-preview ${c.imageUrl ? 'visible' : ''}" id="cf-image-preview" src="${esc(c.imageUrl || '')}" />
    ...
    <div class="field">
      ...
      <input class="search-input" id="cf-product-search" placeholder="Search products…" style="width:100%;margin-bottom:8px;" />
      <div id="cf-product-list" style="max-height:220px;overflow-y:auto;border:1px solid var(--border);border-radius:8px;padding:6px;">
        ${comboProductPickerHTML(selectedIds)}
      </div>
    </div>
    <div class="checkbox-row" style="margin-top:16px;"><input type="checkbox" id="cf-featured" ${c.featured ? 'checked' : ''} /><label for="cf-featured">Mark as Featured (shown first, with a Featured badge)</label></div>
    <div class="checkbox-row"><input type="checkbox" id="cf-active" ${c.active !== false ? 'checked' : ''} /><label for="cf-active">Active (visible on storefront)</label></div>
  `;
}

// Live-updates the disabled "You Save" preview as the admin types either price.
function wireComboFormEvents() {
  const orig = $('cf-original-price');
  const comboP = $('cf-combo-price');
  const savings = $('cf-savings');
  function updateSavingsPreview() {
    const s = Math.max((parseFloat(orig.value) || 0) - (parseFloat(comboP.value) || 0), 0);
    savings.value = money(s);
  }
  if (orig) orig.addEventListener('input', updateSavingsPreview);
  if (comboP) comboP.addEventListener('input', updateSavingsPreview);

  const searchEl = $('cf-product-search');
  if (searchEl) {
    searchEl.addEventListener('input', () => {
      const q = searchEl.value.trim().toLowerCase();
      $('cf-product-list').querySelectorAll('.sf-product-row').forEach((row) => {
        row.style.display = !q || row.getAttribute('data-name').includes(q) ? '' : 'none';
      });
    });
  }
...
function readSelectedComboProductIds() {
  return Array.from($('cf-product-list').querySelectorAll('.cf-product-check:checked')).map((el) => el.value);
}
...
$('add-combo-btn').addEventListener('click', () => {
  openModal('Add Combo Meal', comboFormHTML(), async () => {
    ...
    const name = $('cf-name').value.trim();
    const productIds = readSelectedComboProductIds();
    ...
    const originalPrice = parseFloat($('cf-original-price').value) || 0;
    const comboPrice = parseFloat($('cf-combo-price').value) || 0;
    const items = productIds.map((id) => {
      const p = productsCache.find((x) => x.id === id);
      return { id, name: p ? p.name : '(deleted product)' };
    });
    const data = {
      name,
  ...
  $('cf-image-file').addEventListener('change', previewComboImage);
  wireComboFormEvents();
});
...
document.addEventListener('click', (e) => {
  ...
  if (editId) {
    ...
    openModal('Edit Combo Meal', comboFormHTML(c), async () => {
      ...
      if (file) data.imageUrl = await uploadToCloudinary(file);
      await updateDoc(doc(db, 'combos', editId), data);
  ...
  if (toggleId) {
    ...
    if (c) {
      updateDoc(doc(db, 'combos', toggleId), { active: !(c.active !== false), updatedAt: serverTimestamp() })
        .then(() => toast(c.active !== false ? 'Combo hidden.' : 'Combo shown.'))
        .catch((err) => { toast('Update failed.'); console.error(err); });
    }
  }

  const delId = e.target.closest('[data-delete-combo]')?.getAttribute('data-delete-combo');
  if (delId && confirmAction('Delete this combo meal? This cannot be undone.')) {
    deleteDoc(doc(db, 'combos', delId))
...
function startListeners() {
  ...
  unsubscribers.push(onSnapshot(collection(db, 'combos'), (snap) => {
    combosCache = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
    renderCombosTable();
  }, (err) => console.warn('combos listener error:', err)));
