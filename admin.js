...
async function uploadProductImage(productId, file, oldImageUrl) {
  ...
  try {
    ...
    throw new Error('Image upload failed — could not reach Cloudinary.');
  }
  if (!response.ok) {
    let detail = '';
  ...
  if (!result.secure_url) throw new Error('Image upload failed — Cloudinary did not return a URL.');
